...
@php
    ...
    $filterKeys = ['school_year', 'grade_level', 'section_search', 'status', 'ready_for_review', 'subject_id', 'grading_period', 'awaiting_decision', 'student_search', 'risk_level'];
...
<div class="card overflow-x-auto">
    <div class="px-5 py-4 border-b border-line">
        <h3 class="card-title">
            ...
            <span class="ml-2 text-xs font-normal text-gray-500">School Year: {{ $schoolYear }}</span>
            @if($isHistoricalYear)
                <span class="badge badge-gray ml-1"><i class="bi bi-archive"></i> Historical Record</span>
            @endif
        </h3>
        ...
        <div class="flex flex-wrap items-end gap-3 mt-3 pt-3 border-t">
            <form method="GET" id="interventionFilterForm" class="flex flex-wrap items-end gap-3">
                ...
                <div>
                    ...
                    <p id="ivTrackDisplay" class="form-select-sm min-w-[130px] bg-gray-50 text-gray-600">
                        {{ $selectedSection->track->name ?? 'Not set' }}
                    </p>
                </div>
                <div>
                    <label class="form-label">Specialization</label>
                    <p id="ivSpecializationDisplay" class="form-select-sm min-w-[150px] bg-gray-50 text-gray-600">
                        {{ $selectedSection->specialization->name ?? 'Not set' }}
                    </p>
                </div>
                <div>
                    <label class="form-label">Status</label>
                    <select name="status" class="form-select-sm min-w-[140px]">
                ...
                <div>
                    ...
                    <input type="text" name="student_search" value="{{ request('student_search') }}"
                           ...
                           class="form-select-sm min-w-[150px]">
                </div>
                <div>
                    <label class="form-label">Risk Level</label>
                    <select name="risk_level" class="form-select-sm min-w-[130px]">
                ...
                @if($filtersActive)
                    <a href="{{ route('principal.interventions') }}" class="text-sm text-muted hover:underline pb-1.5">Clear</a>
                @endif
            </form>
        </div>
    ...
    <p class="text-xs text-muted px-6 pt-1">
        ...
        <tbody>
            ...
            <tr class="align-top">
                <td class="font-medium text-ink whitespace-nowrap">
                    ...
                    {{-- PART 12 — the section AT THE TIME the intervention was
                         recorded (interventions.section_id), never the learner's
                         current section. --}}
                    <span class="block text-xs text-muted font-normal">
                        {{ $iv->section->name ?? '—' }}
                        @if($iv->section) (Grade {{ $iv->section->grade_level }}) @endif
                    </span>
                    <span class="block text-xs text-muted font-normal">
                        {{ $iv->section->track->name ?? 'Not set' }} / {{ $iv->section->specialization->name ?? 'Not set' }}
                    </span>
                    <span class="block text-xs text-muted font-normal">
                ...
                <td class="min-w-[220px]">
                    ...
                    <form method="POST" action="{{ route('principal.interventions.update', $iv->id) }}" class="flex items-end gap-2 mt-2">
                        @csrf
                        @method('PUT')
                        <select name="status" class="form-select-sm text-xs">
                            @foreach($statuses as $s)
                    ...
                    <div id="rfr-{{ $iv->id }}" class="mt-2 bg-amber-50 border border-amber-200 rounded-md p-2 text-xs text-amber-800 max-w-[220px]">
                        ...
                        <div class="flex flex-wrap items-center gap-1.5 mt-1.5">
                            <form method="POST" action="{{ route('principal.interventions.update', $iv->id) }}">
                                ...
                                <button type="submit" class="btn btn-primary btn-xs">Completed</button>
                            </form>
                            <form method="POST" action="{{ route('principal.interventions.update', $iv->id) }}">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="status" value="monitoring">
                                <button type="submit" class="btn btn-warning btn-xs">Monitoring</button>
                            </form>
    ...
    <div class="px-5 py-4 border-t border-line flex flex-col items-center gap-2 text-sm text-muted">
        <div class="flex items-center gap-1">
            ...
            @else
                <a href="{{ $interventions->previousPageUrl() }}" class="px-3 py-1 rounded-md border border-line hover:bg-surface text-gray-600">← Prev</a>
            @endif
            <span class="px-3 py-1 rounded-md border border-brand-800 bg-brand-800 text-white font-medium">{{ $interventions->currentPage() }}</span>
            @if($interventions->hasMorePages())
                <a href="{{ $interventions->nextPageUrl() }}" class="px-3 py-1 rounded-md border border-line hover:bg-surface text-gray-600">Next →</a>
            @else
